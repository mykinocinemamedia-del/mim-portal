...
export default async function EmployerNotificationsPage() {
  ...
  const notifications = await db.notification.findMany({
    where: {
      OR: [{ employerId: session.id }, { userId: session.id, userType: 'employer' }],
    },
          ...
          <Card className="border-0 shadow-md">
            <CardContent className="p-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Anda belum mempunyai sebarang notifikasi. Notifikasi akan muncul di
                sini apabila ada maklumat baru.
              </p>
            </CardContent>
          </Card>
        ) : (
            ...
            {notifications.map((n) => (
              <Card
                ...
                className={`border-0 shadow-sm transition ${
                  n.isRead
                    ? 'bg-background'
                    : 'bg-amber-50/30 ring-1 ring-amber-200'
                }`}
              >
