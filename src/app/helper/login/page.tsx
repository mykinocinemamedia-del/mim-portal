...
import { Home, ArrowLeft, LogIn, User } from 'lucide-react'
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({ email, password, role: 'helper' }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)

      toast({ title: 'Log Masuk Berjaya', description: `Selamat datang, ${data.user.name}!` })
      // Use window.location for full page reload so session cookie is properly read
      window.location.href = '/helper/dashboard'
    } catch (e: any) {
      toast({ title: 'Log Masuk Gagal', description: e.message, variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden p-4 bg-[#0d1f33]">
      ...
      <Card className="w-full max-w-md border border-white/10 shadow-2xl relative z-10 glass-dark text-white">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl text-white">Log Masuk Pembantu</CardTitle>
          <CardDescription className="text-slate-400">Masukkan kredensial yang dihantar melalui WhatsApp</CardDescription>
        </CardHeader>
        <form onSubmit={submit}>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email" className="text-slate-300">Email</Label>
              <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="email@mim.com.my" required className="bg-white/5 border-white/10 text-white placeholder:text-slate-500" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="password" className="text-slate-300">Password</Label>
              <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required className="bg-white/5 border-white/10 text-white placeholder:text-slate-500" />
            </div>
            <div className="bg-[#00bcd4]/10 border border-[#00bcd4]/20 rounded-lg p-3 text-xs text-[#00bcd4]">
              ...
              <p>Email: siti.demo@mim.com.my</p>
          ...
          <CardFooter className="flex flex-col gap-3">
            <Button type="submit" className="w-full btn-rounded bg-[#00bcd4] hover:bg-[#00a5bb] text-white border-0" disabled={loading}>
              {loading ? 'Sedang log masuk...' : <><LogIn className="w-4 h-4 mr-2" /> Log Masuk</>}
            </Button>
            <div className="flex justify-between w-full text-sm">
              <Link href="/" className="text-slate-400 hover:text-[#00bcd4] inline-flex items-center">
                <ArrowLeft className="w-3 h-3 mr-1" /> Laman Utama
              </Link>
              <Link href="/helper/register" className="text-[#00bcd4] hover:underline inline-flex items-center">
                <User className="w-3 h-3 mr-1" /> Daftar Baru
              </Link>
            </div>
          </CardFooter>
        </form>
